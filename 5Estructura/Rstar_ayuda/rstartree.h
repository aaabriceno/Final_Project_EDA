...
/*!
 * \brief Clase de plantilla RStarTree
...
private:
  ...
  Node *choose_subtree(Node *node, const BoundingBox &box) {
      ...
      if (static_cast<Node *>(node->items[0])
          ...
          std::copy(node->items.begin(), node->items.end(),
                          std::back_inserter(overlap_preferable_nodes));
      }
